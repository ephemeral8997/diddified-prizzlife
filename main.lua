...
local ExecuteSegment = function(segment)
    ...
    elseif cm("kill") or cm("oof") or cm("die") or cm("k") then
        local DaPlayer = PlrFromArgs(Args[2], false)
        if DaPlayer then
            KillPL(DaPlayer)
    ...
    elseif cm("touchfling") or cm("tfling") then
        if not States.TouchFling then
            ...
            task.spawn(function()
                local interval = 0.2 -- How often to apply fling
                    ...
                    profile("TFling", function()
                        ...
                        if now - lastTick >= interval then
                            ...
                            if hrp then
                                ...
                                hrp:ApplyImpulse(flingDirection * forceMagnitude * mass)
                                lastTick = now
                            end
                        else
                            task.wait()
...
task.spawn(function()
    ...
    Teams.GuardButton.MouseButton1Click:Connect(function()
        ...
        local guardTeam = Services.Teams:FindFirstChild("Guards")
        if guardTeam and typeof(guardTeam.GetPlayers) == "function" then
            if #guardTeam:GetPlayers() > 7 then
                Notif("Cannot join team.", "Guards team is full!", 6)
            end
        else
            warn("Guards team missing or tampered with.")
        end
    end)
